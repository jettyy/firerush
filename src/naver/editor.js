...
import { buildIntroHtml, buildBodyHtml, htmlToPlainText } from '../content/html.js';
...
export async function publishDraft({ post, thumbnailPath, jobId = '' }) {
  ...
  try {
    ...
    const bodyMode = await pasteHtml(page, scope, buildBodyHtml(post));
    logger.info(`본문 입력 완료 (${bodyMode})`, { jobId });
